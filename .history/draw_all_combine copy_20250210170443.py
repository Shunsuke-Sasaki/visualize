...
for target in targets:
    ...
    # 正規化の基準として、LRモデルの値を使用
    if temp_data['LR'] is not None:
        lr_inter, lr_inter_err, lr_extra, lr_extra_err = temp_data['LR']
        norm_factor_inter = lr_inter if lr_inter != 0 else 1
        norm_factor_extra = lr_extra if lr_extra != 0 else 1
    else:
        norm_factor_inter = 1
        norm_factor_extra = 1

    # 各モデルごとに、LRを基準とした正規化値を算出
    for model in models:
...
with PdfPages(pdf_path) as pdf:
    ...
    offsets = [(i - (n_models - 1) / 2) * bar_width for i in range(n_models)]
    # 外挿用の色設定（LR: orange, NN: cyan, SR: lime）
    colors_extra = {'LR': 'orange', 'NN': 'cyan', 'SR': 'lime'}
