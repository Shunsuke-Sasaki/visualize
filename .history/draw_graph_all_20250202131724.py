...
for target in targets:
    ...
    ax.set_ylabel(f'RMSE [{rmse_units[target]}]', fontsize=24)  # 単位を追加
    ax.set_title(f'RMSE Comparison for {target}', fontsize=28)

    ax.set_xticks(index)
    ax.set_xticklabels(df['Model'], fontsize=22)  # x軸のフォントサイズを大きく
    ax.set_yticklabels(
        ax.get_yticks(), fontsize=22
    )  # y軸のフォントサイズを大きく

    ax.legend(
        ['Interpolation RMSE', 'Extrapolation RMSE'], fontsize=22
    )  # 凡例のフォントサイズ
